...
Adapted from: https://github.com/jamesdolezal/slideflow/blob/master/slideflow/model/extractors/ctranspath.py
"""
...
from timm.layers import DropPath, Mlp, PatchEmbed, _assert, lecun_normal_, to_2tuple, trunc_normal_
...
def _build_ctranspath_model():
    ...
    return _create_swin_transformer("swin_tiny_patch4_window7_224", pretrained=False, **model_kwargs)
    ...
    def __init__(
        ...
        if self.shift_size > 0:
            ...
            mask_windows = window_partition(
                img_mask, (self.window_size, self.window_size)
...
#     def __init__(self, device="cuda", head: nn.Module | None = None):
#         self.device = device
#         self.model = _build_ctranspath_model()
#         self.model.head = torch.nn.Identity().to(device) if head is None else head.to(device)

#         checkpoint_path = hf_hub_download(repo_id="jamesdolezal/CTransPath", filename="ctranspath.pth")
