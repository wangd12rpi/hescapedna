    ...
    **Attributes**:
    ...
    def _find_and_replace(self, adapter_name):
        ...
        for key in key_list:
            ...
            layer_index = int(match.group(1)) if match else 0
            print(match)
...
class LoraLayer:
    ...
    def update_layer_conv2d(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        ...
        if isinstance(r, list):
            r = r[0]  # 或 r[layer_index]，强制转为 int
        print(r)
        if r > 0:
            self.r[adapter_name] = r
            self.lora_alpha[adapter_name] = lora_alpha
...
class LoraMoE_Layer:
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, layer_index): ### modified2
        ...
        if isinstance(r, list): ### modified2
            ##newmodified
            # 安全获取 r_val 和 num_experts
            r_val = r[layer_index] if layer_index < len(r) else (r[-1] if r else 0)
            if isinstance(self.number_experts, list):
                num_experts = self.number_experts[layer_index] if layer_index < len(self.number_experts) else (self.number_experts[-1] if self.number_experts else 0)
            else:
                num_experts = self.number_experts
    
            if r_val > 0 and num_experts > 0:
                print("LINEAR", num_experts)
                # 支持 per-layer top_k，如果 forward 中需要（这里暂不创建，但可扩展）
                if hasattr(self, 'top_k') and isinstance(self.top_k, list):
                    top_k_val = self.top_k[layer_index] if layer_index < len(self.top_k) else (self.top_k[-1] if self.top_k else 1)
                else:
                    top_k_val = getattr(self, 'top_k', 1)  # 默认1，如果未定义
    
                self.router.update(nn.ModuleDict({'router': nn.Linear(self.in_features, num_experts, bias=False)}))
                for i in range(num_experts):
                    adapter_name_moe = adapter_name + '_' + str(i)
                    self.lora_A.update(nn.ModuleDict({adapter_name_moe: nn.Linear(self.in_features, r_val, bias=False)}))
                    self.lora_B.update(nn.ModuleDict({adapter_name_moe: nn.Linear(r_val, self.out_features, bias=False)}))
                self.scaling[adapter_name] = lora_alpha / r_val
            else:
                print(f"Skipping MoE-LoRA for layer {layer_index} (r_val={r_val}, num_experts={num_experts})")
        else:
            if r > 0:
                self.lora_A.update(nn.ModuleDict({adapter_name: nn.Linear(self.in_features, r, bias=False)}))
                self.lora_B.update(nn.ModuleDict({adapter_name: nn.Linear(r, self.out_features, bias=False)}))
                ...
                print(f"Skipping standard LoRA for r={r}")
        ...
        if init_lora_weights:
            if isinstance(r, list):
                ...
                if r_val > 0:
                    ...
                    print(f"Skipping MoE reset for layer {layer_index} (r_val={r_val})")
            else:
                if r > 0:
                    self.reset_lora_parameters(adapter_name)
                else:
                    print(f"Skipping standard reset for r={r}")
...
class Linear_MoE(nn.Linear, LoraMoE_Layer):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # 获取 active_adapter 和 r_val
        adapter_name = self.active_adapter
        r = self.r[adapter_name]
        layer_index = getattr(self, 'layer_index', 0)  # 从 __init__ 保存，确保传入
        if isinstance(r, list):
            r_val = r[layer_index] if layer_index < len(r) else (r[-1] if r else 0)
        else:
            r_val = r
    
        # 原 Linear forward（始终计算，作为 base）
        result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
    
        if r_val > 0 and 'router' in self.router and not self.merged:
            ##newmodified
            ### modified2
            batch_size, sequence_length, hidden_dim = x.shape
            x_flat = x.view(-1, hidden_dim)
            #print(self.router)  # 调试打印，生产时可移除
            router_logits = self.router['router'](x_flat)
            ...
            for expert_idx in range(self.number_experts):
                ...
                current_state = x_flat[None, top_x_list].reshape(-1, hidden_dim)
                expert_output = (
            ...
            # router_logits 只用于调试，不返回
    
        result = result.to(previous_dtype)
    
        return result  # 只返回 tensor，不返回 tuple
