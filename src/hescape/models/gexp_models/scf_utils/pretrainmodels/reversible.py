...
def route_args(router, args, depth):
    ...
    for key in matched_keys:
        val = args[key]
        for depth, ((f_args, g_args), routes) in enumerate(zip(routed_args, router[key], strict=False)):
...
class _ReversibleFunction(Function):
    ...
    def forward(ctx, x, blocks, args):
        ctx.args = args
        for block, kwarg in zip(blocks, args, strict=False):
            x = block(x, **kwarg)
        ctx.y = x.detach()
        ctx.blocks = blocks
        return x
    ...
    def backward(ctx, dy):
        y = ctx.y
        args = ctx.args
        for block, kwargs in zip(ctx.blocks[::-1], args[::-1], strict=False):
...
class SequentialSequence(nn.Module):
    ...
    def forward(self, x, output_attentions=False, **kwargs):
        ...
        layers_and_args = list(zip(self.layers, args, strict=False))
        ...
        for (f, g), (f_args, g_args) in layers_and_args:
            ...
            x = x + g(x, **g_args)
        if output_attentions:
            attn_weights = torch.transpose(
                torch.cat(attn_weights, dim=0), 0, 1
            )  # the final dim is (batch, layer, head, len, len)
