...
def causal_linear_attention_noncuda(q, k, v, chunk_size=128):
    ...
    for q, k, v in zip(*map(lambda t: t.chunk(chunk_size, dim=-2), (q, k, v)), strict=False):
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, pos_emb=None, context=None, mask=None, context_mask=None, output_attentions=False, **kwargs):
        ...
        out = torch.cat(
            attn_outs, dim=1
        )  # combine attn_out and cross_attn_out, here we have only attn_out, that means this line does nothing
...
class Performer(nn.Module):
    def __init__(
        ...
        assert len(local_attn_heads) == depth, (
            "tuple specifying number of local attention heads per depth must be equal to the total depth"
        )
        ...
        for _, local_heads in zip(range(depth), local_attn_heads, strict=False):
            layers.append(
                nn.ModuleList(
                    [
                        wrapper_fn(
                        ...
                        wrapper_fn(
                            Chunk(
                                ff_chunks, FeedForward(dim, mult=ff_mult, dropout=ff_dropout, glu=ff_glu), along_dim=1
                            )
                        ),
                    ]
                )
        ...
        route_attn = (
            ((True, False),) * depth * (2 if cross_attend else 1)
