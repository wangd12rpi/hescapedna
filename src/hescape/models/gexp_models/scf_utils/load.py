...
from hescape.models.gexp_models.scf_utils.pretrainmodels import select_model
...
def main_gene_selection(X_df, gene_list):
    ...
    padding_df = pd.DataFrame(
        np.zeros((X_df.shape[0], len(to_fill_columns))), columns=to_fill_columns, index=X_df.index
    )
    X_df = pd.DataFrame(
...
def getEncoerDecoderData(data, data_raw, config):
    ...
    encoder_data, encoder_data_padding = gatherData(decoder_data, encoder_data_labels, config["pad_token_id"])
    new_data_raw = data_raw
    data_gene_ids = torch.arange(data.shape[1], device=data.device).repeat(data.shape[0], 1)
    encoder_position_gene_ids, _ = gatherData(data_gene_ids, encoder_data_labels, config["pad_token_id"])
