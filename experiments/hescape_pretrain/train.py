...
def main(cfg: DictConfig) -> None:
    ...
    base_output_value = cfg.paths.get("base_output_path", None)
    base_output_root = Path(base_output_value) if base_output_value is not None else Path(cfg.paths.anatomy.output).parent
    dataset_slug = cfg.paths.anatomy.get("dataset_name", Path(cfg.paths.anatomy.output).name)
    run_stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_root = base_output_root / f"{dataset_slug}_{run_stamp}"
    cfg.paths.anatomy.output = str(run_root / wandb_name)
